<template>
  ...
  <p>id 具有唯一性，能直接對應到所屬項目，因此當陣列內容發生變動時，只需要處理增減的資料即可，其他資料仍可繼續複用，不需要重新渲染，進而維持效能。</p>
  <pre>
    &lt;li v-for="item in datas" :key="item.id">
  </pre>
  <p>index 會跟著陣列內容的長度變化而隨之增減，且 index 具有順序性，所以只要變動其中一筆資料便會連帶影響到其他資料的排序，使得資料必須隨時重新渲染，相對影響效能</p>
  <pre>
